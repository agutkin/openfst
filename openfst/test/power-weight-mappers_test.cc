...
class ToPowerWeightMapperTest : public testing::Test {
 public:
  using Mapper = ToPowerWeightMapper<TropicalWeight, PowerWeightT>;
};
...
class FromPowerWeightMapperTest : public testing::Test {
 public:
  ...
  using Mapper = FromPowerWeightMapper<PowerWeightT, TropicalWeight>;
};
...
class ProjectPowerWeightMapperTest : public testing::Test {
 public:
  ...
  using Mapper = ProjectPowerWeightMapper<PowerWeightT>;
};
...
SparsePowerWeight<W, K> Transform(const SparsePowerWeight<W, K>& w) {
  ...
  for (Iterator iter(w); !iter.Done(); iter.Next()) {
    result.SetValue(iter.Value().first + 1, W(iter.Value().second.Value() * 2));
  }
  return result;
}
...
PowerWeight<W, n> Transform(const PowerWeight<W, n>& w) {
  ...
  for (size_t i = 0; i + 1 < n; ++i) {
    result.SetValue(i + 1, W(w.Value(i).Value() * 2));
  }
  return result;
}
...
TYPED_TEST(TransformPowerWeightMapperTest, Transform) {
  ...
  EXPECT_THAT(
      ToVector(TransformPowerWeightMapper<Weight, decltype(transform)>(
          transform)(w)),
