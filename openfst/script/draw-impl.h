...
class FstDrawer {
 public:
  ...
  FstDrawer(const Fst<Arc>& fst, const SymbolTable* isyms,
            ...
            int precision, absl::string_view float_format, bool show_weight_one,
 ...
 private:
  void DrawDot(std::ostream& strm, absl::string_view dest) {
    SetStreamState(strm);
  ...
  void DrawD2(std::ostream& strm, absl::string_view dest) {
    SetStreamState(strm);
    dest_ = std::string(dest);
    const auto start = fst_.Start();
    if (start == kNoStateId) return;
